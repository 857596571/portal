...
const CreateForm = Form.create({
  ...
  const props2 = !mapPropsToFields.imgUrl ||addOrEdit? {
    action: '/api/advertisement/upload',
    listType: 'picture',
    ...
    >
      ...
      >
        {form.getFieldDecorator('url', {
          rules: [{max:200,message:'最大长度不能超过200字符'},{type:'url',message:'必须输入网址'}],
        })(
          <Input placeholder="请输入" />
        )}
      </FormItem>
      ...
      >
        {form.getFieldDecorator('imgUrl', {
          ...
          <Upload { ...props2 }>
            <i style={{fontSize:'12px',color:'red'}}>建议图片大小为：1600*400</i><br/>
            <Button >
...
export default class Advertisement extends PureComponent {
  ...
  sortNum = (e, key, row) =>{
    ...
    let type = 'advertisement/sortDown';
    if(key == 0){
      type = 'advertisement/sortUp';
    }
    if(key == 3) {
      type = 'advertisement/sortFirst';
    }
    dispatch({
      type: type,
      payload:{id:row.id},
  ...
  render() {
    const { advertisement: { data }, loading } = this.props;
