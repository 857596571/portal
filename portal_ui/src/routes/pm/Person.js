...
const CreateForm = Form.create({
  mapPropsToFields(props) {
    return {
      ...
      post: Form.createFormField({value: props.mapPropsToFields.post}),
      detail: Form.createFormField({value: props.mapPropsToFields.detail}),
    ...
    >
      ...
      >
        {form.getFieldDecorator('detail', {
          rules: [{ required: true, message: '请上传图片' },{max:200,message:'最大长度不能超过200字符'}],
        })(
          <TextArea rows={4} />
        )}
      </FormItem>
      ...
      >
        {form.getFieldDecorator('imgUrl', {
          ...
          <Upload { ...props2 }>
            <i style={{fontSize:'12px',color:'red'}}>建议图片比例为1：1(98px*98px)</i><br/>
            <Button>
...
export default class Person extends PureComponent {
  ...
  sortNum = (e, key, row) =>{
    ...
    let type = 'person/sortDown';
    if(key == 0){
      type = 'person/sortUp';
    }
    if(key == 3){
      type = 'person/sortFirst';
    }
    dispatch({
      type: type,
      payload:{id:row.id},
